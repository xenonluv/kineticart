...
### 임베딩이 왜 필요한가 — 예시로 이해하기

키워드 검색은 **글자**가 겹쳐야 찾는다. "잔잔한 물결 모양"으로 검색해도 설명에 '물결'이라는 글자가 없으면 못 찾는다.
**임베딩**은 문장을 **숫자 좌표(1024차원)** 로 바꿔 거대한 "의미 지도" 위의 점으로 만든다 — **뜻이 비슷하면 점이 가깝다.** 그래서 단어가 안 겹쳐도 의미로 찾는다.

실제로 이 프로젝트 임베딩으로 검색한 결과(질의어와 단어가 겹치지 않아도 **뜻으로** 매칭됨):

| 질문(검색어) | 찾아온 작품 | 왜 (뜻이 가까움) |
|---|---|---|
| "물처럼 흐르는 금속 조각" | 물의 이중주 분수, 금속 야외조각 | '물·흐름' 의미가 가까움 |
| "잔잔한 물결 모양" | 동심원 옵아트, 물 분수 | 물결·동심원이 뜻으로 근처 |
| "빛과 색이 변하는 설치" | 크로마 VII (빛·구조색) | '빛·색' 의미가 가까움 |
| "모터로 회전하는 기계" | 에오스, 모듈레이터 V3 | '모터·회전' 의미가 가까움 |
...
|---|---|---|
| 보는 것 | **글자** 일치 | **뜻** 가까움 |
| "잔잔한 물결" 검색 | 그 글자 없으면 실패 | 물결·동심원·분수 찾아냄 |
| 비유 | 사전에서 단어 찾기 | 지도에서 가까운 동네 찾기 |

동작 순서: ① 모든 작품 설명을 미리 임베딩해 "지도에 점"으로 찍어둠(5b단계) → ② 사용자 질문도 점으로 변환 → ③ 가장 가까운 점들을 반환(**가까움 = 비슷한 뜻**).
